...
func TestGetItemsDueForReview(t *testing.T) {
	testutils.Run(t, testutils.Level2, "TestGetItemsDueForReview", nil, func(t *testing.T, tx *gorm.DB) {
		dbPath := "./test_due_for_review.db"
		defer os.Remove(dbPath)
		...
		items, err := ldb.GetItemsDueForReview(10)
		if err != nil {
			t.Fatalf("Failed to get items due for review: %v", err)
		}

		if items == nil {
			t.Error("GetItemsDueForReview should not return nil")
		}
	})
}
