...
export function CWETable({ viewMode: propViewMode = 'view' }: CWETableProps) {
  const { mode: contextMode } = useViewLearnMode();
  const viewMode = propViewMode || contextMode;
  const isLearnMode = viewMode === 'learn';
            ...
            <table className="min-w-full text-xs">
              ...
              <tbody>
                ...
                {cweList.map((cwe: any, idx: number) => (
                  <tr key={cwe.id || idx} className={`border-b hover:bg-muted/30 ${isLearnMode ? 'learn-enhanced' : ''}`}>
                  ...
                  >
                    ...
                    <div className="mb-4 flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <h2 className={`text-lg font-medium ${isLearnMode ? 'learn-enhanced' : ''}`}>CWE-{detailCWE.id || detailCWE.ID}: {detailCWE.name || detailCWE.Name}</h2>
                        <div className={`mt-1 flex flex-wrap items-center gap-2 text-sm text-muted-foreground ${isLearnMode ? 'learn-minimal' : ''}`}>
                    ...
                    <div className="mb-4">
                      <div className="text-xs text-muted-foreground font-mono bg-muted px-2 py-1 rounded inline-block">
                        URN: {generateURN('CWE', detailCWE.id || detailCWE.ID)}
                      </div>
                    </div>
                    <section className={`mb-4 ${isLearnMode ? 'learn-enhanced' : ''}`}>
                      <h3 className="font-semibold mb-2">Description</h3>
                      <p className={`text-sm whitespace-pre-wrap ${isLearnMode ? 'learn-text-lg' : ''}`}>{detailCWE.description || detailCWE.Description}</p>
                    </section>
