...
1. **Missing Initialize Method** (Status: ⚠️ KNOWN ISSUE)
   - **Problem**: `fsm.ProviderFSM` interface doesn't have `Initialize()` method
   - **Impact**: Providers cannot set up context before starting
   - **Workaround**: Context set in provider constructor
   - **Fix**: Add `Initialize(context.Context) error` to `fsm.ProviderFSM` interface

2. **Checkpoint Storage Behavior** (Status: ✅ EXPECTED BEHAVIOR)
   - **Problem**: BoltDB uses URN as key, duplicate checkpoints overwrite
   - **Impact**: Only last checkpoint per unique URN is persisted
   - **Expected**: This is correct idempotent behavior
   - **Note**: Tests should expect 1 checkpoint per unique URN, not 5

3. **Missing GetStats Method** (Status: ⚠️ KNOWN ISSUE)
   - **Problem**: `fsm.ProviderFSM` interface doesn't have `GetStats()` method
   - **Impact**: Cannot retrieve provider statistics via RPC
   - **Fix**: Add `GetStats() map[string]interface{}` method to interface

4. **Event Handler Required** (Status: ✅ ALREADY IMPLEMENTED)
   - **Solution**: MacroFSMManager sets event handler when adding provider
   - **Implementation**: See `macroFSM.AddProvider()` in `pkg/meta/fsm/macro.go:234`

5. **Message UnmarshalParams Method Missing** (Status: ⚠️ KNOWN ISSUE)
   - **Problem**: `subprocess.Message` doesn't have `UnmarshalParams` method
   - **Impact**: Cannot parse RPC parameters in handlers
   - **Fix**: Use direct parameter extraction or add method to Message
